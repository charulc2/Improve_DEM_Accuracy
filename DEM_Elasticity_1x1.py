...
filename="1x1_Displacements_trained4x1"
...
if __name__ == '__main__':
    ...
    fig_aspect_r=Height_test/Length_test
    ...
    abaqus_res= pd.read_excel("/u/charulc2/HOpt_Feb2023/Abaqus_Displacements.xlsx", sheet_name= "Comp1x1_10Mpa")
    shape_ab=[25,25]
    ...
    cp = ax10.contourf(x_new_coords_ab,y_new_coords_ab.reshape(shape_test[1],shape_test[0]),
                       Ux_ab.reshape(shape_test[1],shape_test[0]),12,cmap='jet')
    ax10.set_title('FEM: $U_x$')  
    ax10.set_aspect(fig_aspect_r)
    fig.colorbar(cp,shrink=0.4,location = 'left', ax=ax10)  

    cp2 = ax11.contourf(x_new_coords_ab.reshape(shape_test[1],shape_test[0]),y_new_coords_ab.reshape(shape_test[1],shape_test[0]),
                        Uy_ab.reshape(shape_test[1],shape_test[0]),12,cmap='jet')
    
    ax11.set_title('FEM: $U_y$')
    ax11.set_aspect(fig_aspect_r)
    fig.colorbar(cp2,shrink=0.4,location = 'left', ax=ax11)
    #plt.savefig(filename+"FEM_Disp")
    ...
    ax10.set_title('$Error_x$ = |$U_{x(FEA)}$ - $U_{x(DEM)}$|')
    ...
    ax11.set_title('$Error_y$ = |$U_{y(FEA)}$ - $U_{y(DEM)}$|')
    ax11.set_aspect(fig_aspect_r)
    fig.colorbar(cp2,shrink=0.4,location = 'left', ax=ax11)
    #plt.savefig(filename+"Errors")
