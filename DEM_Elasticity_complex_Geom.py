...
filename="DEM_Elastic_Complex_Geom_Eg"
   ...
   for nodex in range(Nx):
       for nodey in range (Ny):
           if (((train_x_coord[nodey,nodex]-Crcl_x)/E_major)**2+((train_y_coord[nodey,nodex]-Crcl_y)/E_minor)**2<1):
                ...
                density_node[nodey,nodex]=0
   
   plt.imshow(density.cpu(), extent = [x_min, Length, y_min, Height],cmap='gray_r')
   plt.colorbar(shrink=0.5,location = 'left')
   plt.show()
   
   return density , density_node
...
if __name__ == '__main__':
    # Read me a density field 
    density = np.load('Beam.npy').reshape([30,120])
    print( density.shape )

    # Binarize
    thres = 0.35
    density[ density < thres ] = 0 
    density[ density >= thres ] = 1

    # Add a base on the ends to bear load
    density[:,:4] = 1
    density[:,-4:] = 1

    tmp = np.zeros([60,120])
    tmp[:30,:] = density
    tmp[30:,:] = np.flipud( density )
    density = tmp.copy()

    np.save('BinarizedDesign.npy',density)
    plt.matshow( density , cmap='gray' )
    plt.show()
    # exit()

    density_node = np.zeros([61,121])

    for i in range( 60 ):
        for j in range( 120 ):
            if density[i,j] > 0.5:
                density_node[ i+1 , j+1 ] = 1

    
    density = torch.from_numpy( density ).to(dev)
    density_node = torch.from_numpy( density_node ).to(dev)
    ...
    # density , density_node = get_density()
    # density=1
    # ----------------------------------------------------------------------
    #                   STEP 2: SETUP MODEL
    # ----------------------------------------------------------------------
    ...
    density_npy = density_node.cpu().numpy().flatten()
    Ux[ density_npy < 0.5 ] = np.nan
    Uy[ density_npy < 0.5 ] = np.nan
    
    fig_aspect_r=Height_test/Length_test * 2
    ...
    print('Ux_Max= %0.5f, Ux_Min= %0.5f'%(np.nanmax(Ux),np.nanmin(Ux)))
    print('Uy_Max= %0.5f, Uy_Min= %0.5f'%(np.nanmax(Uy),np.nanmin(Uy)))
    ...
    abaqus_res= pd.read_excel("/u/charulc2/HOpt_Feb2023/Abaqus_Displacements.xlsx", sheet_name= "Complex_Geom")
    shape_ab=[121,61]
    ...
    Ux_ab= np.where(Ux_ab ==0, 'nan', Ux_ab)
    Uy_ab= np.where(Uy_ab ==0, 'nan', Uy_ab)
    ...
    error_Ux= abs(Ux.reshape(shape_test[1],shape_test[0])-Ux_ab.astype(float))
    error_Uy= abs(Uy.reshape(shape_test[1],shape_test[0])-Uy_ab.astype(float))
...
#----------------- Final Plot-------------------
    ...
    fig.tight_layout()
    plt.setp(ax, xticks=[0,2,4,6,8,10,12], yticks=[0,2,4,6])
    ...
    plt.savefig(filename+"Displacements2")
