...
class InternalEnergy_Stress:
    ...
    def Elastic2DGauusStress (self, u, x, dxdydz, shape, density):
        ...
        X_Cord= torch.transpose(x[:, 0].unsqueeze(1).reshape(shape[0], shape[1]), 0, 1)
        Y_Cord= torch.transpose(x[:, 1].unsqueeze(1).reshape(shape[0], shape[1]), 0, 1)
        ...
        x_Centeroid= (xN1+xN2+xN3+xN4).detach().numpy()/4
        y_Centeroid= (yN1+yN2+yN3+yN4).detach().numpy()/4
